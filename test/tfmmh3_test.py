...
class Testtfmmh3( unittest.TestCase ):
    ...
    def test_32bit_custom_seed_string( self ):
        ...
        with open( os.path.join( file_dir, 'pg1260.txt' ), 'rb' ) as test_file:
            for l in test_file.readlines():
                s = solution[l]
                r = tfmmh3.hash( l, seed = 0x1234ABCD )
                self.assertEqual( s, r, 'different hash for line: "%s"\n0x%08X != 0x%08X' % ( l, s, r ) )
    ...
    def test_32bit_custom_seed_bytearray( self ):
        ...
        with open( os.path.join( file_dir, 'pg1260.txt' ), 'rb' ) as test_file:
            for l in test_file.readlines():
                s = solution[l]
                r = tfmmh3.hash( bytearray( l ), seed = 0x1234ABCD )
                self.assertEqual( s, r, 'different hash for line: "%s"\n0x%08X != 0x%08X' % ( l, s, r ) )
    ...
    def test_128bit_x86_basic_string( self ):
        ...
        with open( os.path.join( file_dir, 'pg1260.txt' ), 'rb' ) as test_file:
            for l in test_file.readlines():
                s = solution[l]
                r = tfmmh3.hash128( l , x64arch = False )
                self.assertEqual( s, r, 'different hash for line: "%s"\n0x%08X != 0x%08X' % ( l, s, r ) )
    ...
    def test_128bit_x86_basic_bytearray( self ):
        ...
        with open( os.path.join( file_dir, 'pg1260.txt' ), 'rb' ) as test_file:
            for l in test_file.readlines():
                s = solution[l]
                r = tfmmh3.hash128( bytearray( l ), x64arch = False )
                self.assertEqual( s, r, 'different hash for line: "%s"\n0x%08X != 0x%08X' % ( l, s, r ) )
    ...
    def test_128bit_x86_custom_seed_string( self ):
        ...
        with open( os.path.join( file_dir, 'pg1260.txt' ), 'rb' ) as test_file:
            for l in test_file.readlines():
                s = solution[l]
                r = tfmmh3.hash128( l, seed = 0x1234ABCD, x64arch = False )
                self.assertEqual( s, r, 'different hash for line: "%s"\n0x%08X != 0x%08X' % ( l, s, r ) )
    ...
    def test_128bit_x86_custom_seed_bytearray( self ):
        ...
        with open( os.path.join( file_dir, 'pg1260.txt' ), 'rb' ) as test_file:
            for l in test_file.readlines():
                s = solution[l]
                r = tfmmh3.hash128( bytearray( l ), seed = 0x1234ABCD, x64arch = False )
                self.assertEqual( s, r, 'different hash for line: "%s"\n0x%08X != 0x%08X' % ( l, s, r ) )
    ...
    def test_128bit_x64_basic_string( self ):
        ...
        with open( os.path.join( file_dir, 'pg1260.txt' ), 'rb' ) as test_file:
            for l in test_file.readlines():
                s = solution[l]
                r = tfmmh3.hash128( l, x64arch = True )
                self.assertEqual( s, r, 'different hash for line: "%s"\n0x%08X != 0x%08X' % ( l, s, r ) )
    ...
    def test_128bit_x64_basic_bytearray( self ):
        ...
        with open( os.path.join( file_dir, 'pg1260.txt' ), 'rb' ) as test_file:
            for l in test_file.readlines():
                s = solution[l]
                r = tfmmh3.hash128( bytearray( l ), x64arch = True )
                self.assertEqual( s, r, 'different hash for line: "%s"\n0x%08X != 0x%08X' % ( l, s, r ) )
    ...
    def test_128bit_x64_custom_seed_string( self ):
        ...
        with open( os.path.join( file_dir, 'pg1260.txt' ), 'rb' ) as test_file:
            for l in test_file.readlines():
                s = solution[l]
                r = tfmmh3.hash128( l, seed = 0x1234ABCD, x64arch = True )
                self.assertEqual( s, r, 'different hash for line: "%s"\n0x%08X != 0x%08X' % ( l, s, r ) )
    ...
    def test_128bit_x64_custom_seed_bytearray( self ):
        ...
        with open( os.path.join( file_dir, 'pg1260.txt' ), 'rb' ) as test_file:
            for l in test_file.readlines():
                s = solution[l]
                r = tfmmh3.hash128( bytearray( l ), seed = 0x1234ABCD, x64arch = True )
                self.assertEqual( s, r, 'different hash for line: "%s"\n0x%08X != 0x%08X' % ( l, s, r ) )
